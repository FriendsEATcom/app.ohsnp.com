...
class AccountController extends Controller
{
    ...
    private function save()
    {
        ...
        if (!preg_match("/^([a-z0-9_]+[a-z0-9\.]){1,30}$/", $username)) {
    ...
    protected function checkpoint()
    {
        ...
        if ($resp->status != "ok") {
            $msg = empty($resp->challenge->errors) 
        ...
        try {
            ...
            $this->resp->result = 2;
            $this->resp->msg = __("Please goto <a href='http://instagram.com' target='_blank'>instagram.com</a> and pass challenge!");
            $this->jsonecho();
        } catch (InstagramAPI\Exception\AccountDisabledException $e) {
            $this->resp->msg = __(
                "Your account has been disabled for violating Instagram terms. <a href='%s'>Click here</a> to learn how you may be able to restore your account.", 
